...
public class RuyiInstallManager {
    ...
    private static final String OFFICIAL_REPO_URL = "https://mirror.iscas.ac.cn/ruyisdk/ruyi/releases/";
    private static final String MIRROR_REPO_URL = "https://mirror.iscas.ruyisdk.com";
...
//        String ruyiInstallPath = Paths.get(installPath, latestRelease.getFilename()).toString();
        ...
        String[] downloadSources = {
            latestRelease.getMirrorUrl(),  // 优先尝试镜像地址
        ...
        for (int i = 0; i < downloadSources.length; i++) {
            ...
            try {
                listener.logMessage(String.format(
                    "尝试从%s下载: %s", sourceName, ruyiDownloadUrl));

                RuyiNetworkUtils.downloadFile(
                    ruyiDownloadUrl,
                    ruyiInstallPath.toString(),
                    monitor,
                    (transferred, total) -> {
                        int percent = (int)((double)transferred / total * 100);
                        listener.progressChanged(
                            10 + percent/2,
                            String.format("从%s下载中 (%d/%d KB)", 
                ...
                lastException = e;
                listener.logMessage(String.format(
                    "%s下载失败: %s", sourceName, e.getMessage()));
    ...
    private void downloadRuyi1(IProgressMonitor monitor, InstallationListener listener) throws Exception {
            ...
            try {
                ...
                RuyiNetworkUtils.downloadFile(
                    ...
                    (transferred, total) -> {
                        ...
                        listener.progressChanged(
                            10 + percent / 2,
                            String.format("Downloading from %s (%d/%d KB)", 
        ...
        if (lastException != null) {
            throw new Exception("All download attempts failed. Last error: " + lastException.getMessage(), lastException);
        } else {
    ...
    private void addToPathIfNeeded(String path, InstallationListener listener) throws Exception {
     ...
     // 3. 准备配置命令（包含PATH设置和权限设置）
        String command = String.format(
            "echo 'export PATH=\"%s:$PATH\"' >> ~/.bashrc && " +  // 添加PATH
            "chmod +x %s && " +                                   // 设置可执行权限
            "source ~/.bashrc",                                   // 立即生效
            path,
            ruyiExecutable.toString()
        );
     ...
     // 4. 执行特权命令
        ...
        if (!Files.isExecutable(ruyiExecutable)) {
            ...
            String perms = Files.exists(ruyiExecutable) ? 
                ...
                "File does not exist";
            throw new Exception(String.format(
                "Permission verification failed for: %s\n%s",
                ruyiExecutable,
                perms));
        }
//        listener.logMessage("Successfully set executable permission for: " + ruyiExecutable);
    ...
    private boolean showConfirmationDialog(String title, String message) {
        try {
            // 尝试使用zenity图形对话框
            return new ProcessBuilder("zenity", "--question", 
                    "--title=" + title,
                    "--text=" + message,
                    "--width=300")
                .start().waitFor() == 0;
