...
public class RuyiAPI {
    ...
    public static RuyiReleaseInfo getLatestRelease(String osArch) throws RuyiAPIException {
        try {
            ...
            // 构建版本信息对象
            String[] urls = downloads.getJSONArray(platformKey)
                                  .toList()
                                  .stream()
                                  .map(Object::toString)
                                  .toArray(String[]::new);
...
class RuyiAPIException extends Exception {
    public RuyiAPIException(String message) { super(message); }
    public RuyiAPIException(String message, Throwable cause) { super(message, cause); }
}